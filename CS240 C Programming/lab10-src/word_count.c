...
void wordCount() {
	...
	while((newWord = nextWordInFile())!= NULL){
	  
	  newWord = toLower(newWord);

	  if(nWords == MAX_LIST_LEN - 1){
	    MAX_LIST_LEN = MAX_LIST_LEN * 2;
	    WordList = realloc(WordList, MAX_LIST_LEN * sizeof(struct WordCount));
	  }

	  if(getWordIndex(newWord) < 0){
	    WordList[nWords].word = newWord;
	    WordList[nWords].count = 1;
	    nWords++;
	  }else{
	    WordList[getWordIndex(newWord)].count++;
	  }
...
int main(int argc, char **argv) {
	...
	WordList = (struct WordCount*)malloc(MAX_LIST_LEN * sizeof(struct WordCount));
